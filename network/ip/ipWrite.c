 ...
 */
command ipWrite(uchar *payload, ushort payload_len,ushort ipv4_type, uchar* dst_ip_addr)
{
	static ushort id = 0;
	uchar *mac = malloc(ETH_ADDR_LEN);

	if (SYSERR == arpResolve(dst_ip_addr, mac)) {
		printf("ERROR in ping->arpResolve\n");
		return SYSERR;
	}
	
	//how many fragmented datagrams do we have to produce.
	struct ipgram *ipPkt = malloc(ETH_MTU);
	if(ipPkt==NULL){
		kprintf("ERROR: couldn't allocate memory for ip packet\r\n");
		return SYSERR;
	}
	int fr_grams = payload_len/(ETH_MTU-IPv4_HDR_LEN);
	...
	for(i=0;i<=fr_grams;i++){
		...
		//2nd case payload is exactly 1400 bytes : then len is ETH_MTU
			if(payload_len%(ETH_MTU-IPv4_HDR_LEN)!=0) 
				ipPkt->len = htons((payload_len%(ETH_MTU-IPv4_HDR_LEN))+IPv4_HDR_LEN);
			else
		...
		ipPkt->flags_froff += ((ETH_MTU-IPv4_HDR_LEN)*i)/8;
		ipPkt->id = id;
		...
		memcpy(&ipPkt->opts[0], payload+(ETH_MTU-IPv4_HDR_LEN)*i, ETH_MTU-IPv4_HDR_LEN);
